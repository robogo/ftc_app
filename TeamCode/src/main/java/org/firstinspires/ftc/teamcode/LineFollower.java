...
public class LineFollower extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        whiteThreshold = lightSensor.getLightDetected();
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            while (lightSensor.getLightDetected() < whiteThreshold && !isStopRequested()) {
                robot.leftMotor.setPower(Speed);
                robot.rightMotor.setPower(Speed/2);
            }

            while (lightSensor.getLightDetected() > whiteThreshold && !isStopRequested()) {
                robot.rightMotor.setPower(Speed);
                robot.leftMotor.setPower(Speed/2);
            }
        }
